...
// Limiter para registro y recuperación (anti abuso de códigos por correo)
...
async function inicializarBD(intento = 1) {
    try {
        ...
        try { await pool.execute('ALTER TABLE mensajes ADD COLUMN leido TINYINT(1) DEFAULT 0 AFTER contenido'); } catch(e) {}
        try { await pool.execute('ALTER TABLE perfiles_trabajador ADD COLUMN nivel TINYINT DEFAULT 1 AFTER calificacion_promedio'); } catch(e) {}
        try { await pool.execute('ALTER TABLE perfiles_trabajador ADD COLUMN radio_trabajo_km INT DEFAULT 10 AFTER longitud'); } catch(e) {}
...
// ==========================================================================
// VALIDACIONES DE SEGURIDAD (pre-producción)
// ==========================================================================
function validarPassword(p) {
    if (typeof p !== 'string' || p.length < 8) return 'La contraseña debe tener al menos 8 caracteres.';
    if (!/[A-Za-zÁ-ÿ]/.test(p) || !/\d/.test(p)) return 'La contraseña debe combinar letras y números.';
    if (p.length > 72) return 'La contraseña es demasiado larga.';
    return null;
}
function excedeLargo(valor, max) {
...
app.post('/api/register', authLimiter, async (req, res) => {
    ...
    if (excedeLargo(nombre, 100) || excedeLargo(apellido, 100) || excedeLargo(email, 255)) {
        return res.status(400).json({ message: 'Alguno de los campos excede el largo permitido.' });
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        return res.status(400).json({ message: 'Ingresa un correo electrónico válido.' });
    }
    const errorPass = validarPassword(password);
    if (errorPass) return res.status(400).json({ message: errorPass });
...
app.get('/api/profile', authenticateToken, async (req, res) => {
    try {
        ...
        if (perfil.rol === 'trabajador') {
            ...
            // Sincronizar el nivel (Ruta del Relámpago) y resumen de la wallet
            try { await actualizarNivelTrabajador(req.user.id); } catch (e) {}
...
app.put('/api/trabajador/disponibilidad', authenticateToken, requireRole('trabajador'), async (req, res) => {
    const { disponible, latitud, longitud, radio_km } = req.body;
    const radioValido = [5, 10, 20, 50, 100].includes(parseInt(radio_km)) ? parseInt(radio_km) : null;
    try {
        await pool.execute(
            `UPDATE perfiles_trabajador
             SET disponible = ?, latitud = COALESCE(?, latitud), longitud = COALESCE(?, longitud),
                 radio_trabajo_km = COALESCE(?, radio_trabajo_km)
             WHERE usuario_id = ?`,
            [!!disponible, latitud ?? null, longitud ?? null, radioValido, req.user.id]
        );
        res.status(200).json({ message: disponible ? 'Estás en línea: recibirás trabajos de tu zona.' : 'Estás fuera de línea: no recibirás notificaciones de trabajos.' });
    } catch (error) {
...
app.post('/api/solicitudes', authenticateToken, requireRole('cliente'), async (req, res) => {
    ...
    if (excedeLargo(direccion, 255)) return res.status(400).json({ message: 'La dirección es demasiado larga (máx. 255 caracteres).' });
    if (excedeLargo(descripcion, 2000)) return res.status(400).json({ message: 'La descripción es demasiado larga (máx. 2000 caracteres).' });
    try {
        ...
        // MODO ACTIVO: avisar en vivo SOLO a los trabajadores de esa categoría
        // que estén disponibles y dentro de cuyo radio de trabajo caiga el pedido.
        try {
            const [cercanos] = await pool.execute(
                `SELECT pt.usuario_id, pt.radio_trabajo_km AS radio,
                        CASE
                            WHEN ? IS NOT NULL AND pt.latitud IS NOT NULL THEN
                                ROUND(6371 * ACOS(LEAST(1, GREATEST(-1,
                                    COS(RADIANS(pt.latitud)) * COS(RADIANS(?)) *
                                    COS(RADIANS(?) - RADIANS(pt.longitud)) +
                                    SIN(RADIANS(pt.latitud)) * SIN(RADIANS(?))
                                ))), 1)
                            ELSE NULL
                        END AS distancia_km
                 FROM perfiles_trabajador pt
                 JOIN trabajador_categorias tc ON tc.trabajador_id = pt.usuario_id AND tc.categoria_id = ?
                 JOIN usuarios u ON u.id = pt.usuario_id AND u.estado = 'activo'
                 WHERE pt.disponible = 1
                 HAVING (distancia_km IS NULL OR distancia_km <= radio)
                 LIMIT 50`,
                [latitud || null, latitud || null, longitud || null, latitud || null, categoria_id]
            );
            const [[cat]] = await pool.execute('SELECT nombre FROM categorias_servicio WHERE id = ?', [categoria_id]);
            cercanos.forEach(t => emitirEvento(`user_${t.usuario_id}`, 'nueva_solicitud', {
                solicitud_id: result.insertId,
                categoria_nombre: cat ? cat.nombre : 'servicio',
                distancia_km: t.distancia_km,
                urgencia: urgencia || 'normal'
            }));
        } catch (e) { /* la notificación en vivo nunca debe romper la creación */ }
...
app.get('/api/solicitudes/disponibles', authenticateToken, requireRole('trabajador'), bloquearSiDeudaVencida, async (req, res) => {
    try {
        // MODO ACTIVO (estilo apps de servicio): si el trabajador está fuera de
        // línea, no ve trabajos ni recibe notificaciones hasta que se active.
        const [[perfil]] = await pool.execute(
            'SELECT disponible, radio_trabajo_km FROM perfiles_trabajador WHERE usuario_id = ?',
            [req.user.id]
        );
        if (!perfil || !perfil.disponible) {
            return res.status(200).json({ inactivo: true, radio_km: Number(perfil?.radio_trabajo_km) || 10 });
        }

        // DISTANCIA (Haversine) + FILTRO POR RADIO DE TRABAJO del trabajador.
        // Las solicitudes sin coordenadas (o si el trabajador no tiene GPS)
        // se muestran igual para no ocultar oportunidades.
        const [rows] = await pool.execute(
            `SELECT s.*, c.nombre AS categoria_nombre, c.icono,
                    u.nombre AS cliente_nombre, u.telefono AS cliente_telefono,
                    pt.radio_trabajo_km AS radio_trabajo,
                    CASE
             ...
             HAVING (distancia_km IS NULL OR distancia_km <= radio_trabajo)
             ORDER BY
...
app.get('/api/solicitudes/:id/ofertas', authenticateToken, requireRole('cliente'), async (req, res) => {
    try {
        // VISIBILIDAD POR NIVEL (Ruta del Relámpago): los trabajadores de mayor
        // nivel aparecen primero ante el cliente, luego por calificación.
        const [rows] = await pool.execute(`
            SELECT o.id, o.monto, o.mensaje, u.nombre, u.apellido,
                   pt.calificacion_promedio, pt.total_servicios, pt.verificado,
                   COALESCE(pt.nivel, 1) AS nivel
            ...
            ORDER BY nivel DESC, pt.calificacion_promedio DESC, o.fecha ASC`, [req.params.id]
        );
        // Enriquecer con los datos del nivel para las insignias del cliente
        const enriquecidas = rows.map(r => {
            const cfg = configNivel(Number(r.nivel));
            return { ...r, nivel_nombre: cfg.nombre, nivel_icono: cfg.icono, nivel_color: cfg.color };
        });
        res.status(200).json(enriquecidas);
    } catch (error) {
...
app.put('/api/solicitudes/:id/estado', authenticateToken, requireRole('trabajador'), async (req, res) => {
    ...
    try {
        if (estado === 'completado') {
            ...
            // ============ COMISIÓN SEGÚN EL NIVEL (Ruta del Relámpago) ============
            // Chispa 10% / Rayo 7% / Catatumbo 5%. Se usa el nivel vigente AL
            // COMPLETAR el trabajo; si con este trabajo asciende, el nuevo
            // porcentaje aplica desde el siguiente.
            const [solRows] = await pool.execute(
            ...
            if (montoServicio > 0) {
                const [[perfilNivel]] = await pool.execute('SELECT nivel FROM perfiles_trabajador WHERE usuario_id = ?', [req.user.id]);
                const cfg = configNivel(Number(perfilNivel?.nivel) || 1);
                const montoComision = Math.round(montoServicio * cfg.comision) / 100;
                await pool.execute(
                    `INSERT IGNORE INTO comisiones (solicitud_id, trabajador_id, monto_servicio, porcentaje, monto_comision)
                     VALUES (?, ?, ?, ?, ?)`,
                    [req.params.id, req.user.id, montoServicio, cfg.comision, montoComision]
                );
                comisionInfo = { porcentaje: cfg.comision, monto: montoComision, nivel: cfg.nombre };
            }

            // Con el trabajo completado, revisar si asciende de nivel
            const nivelActualizado = await actualizarNivelTrabajador(req.user.id);
            if (comisionInfo) comisionInfo.nivel_actual = nivelActualizado;
...
app.post('/api/calificaciones', authenticateToken, requireRole('cliente'), async (req, res) => {
    ...
    try {
        ...
        // La nueva calificación puede subir (o bajar) el nivel del trabajador
        await actualizarNivelTrabajador(trabajadorId);
...
app.get('/api/solicitudes/:id/mensajes', authenticateToken, async (req, res) => {
    try {
        ...
        // Al abrir la conversación, los mensajes del otro participante quedan leídos
        await pool.execute(
            `UPDATE mensajes SET leido = 1 WHERE solicitud_id = ? AND remitente_id != ? AND leido = 0`,
...
// Conteo de mensajes no leídos del usuario (para el badge de la pestaña Mensajes)
app.get('/api/mensajes/no-leidos', authenticateToken, async (req, res) => {
    try {
        const [rows] = await pool.execute(
            `SELECT m.solicitud_id, COUNT(*) AS cantidad
             FROM mensajes m
             JOIN solicitudes_servicio s ON s.id = m.solicitud_id
             WHERE m.leido = 0 AND m.remitente_id != ?
               AND (s.cliente_id = ? OR s.trabajador_id = ?)
             GROUP BY m.solicitud_id`,
            [req.user.id, req.user.id, req.user.id]
        );
        const total = rows.reduce((s, r) => s + Number(r.cantidad), 0);
        res.status(200).json({ total, por_solicitud: rows });
    } catch (error) {
        res.status(500).json({ message: 'Error al contar mensajes.' });
    }
});
    ...
    try {
        const [rows] = await pool.execute(`
            SELECT u.id, u.nombre, u.apellido, u.email, u.estado,
                   ...
                   pt.documento_identidad, pt.cancelaciones, COALESCE(pt.nivel, 1) AS nivel
            FROM usuarios u
...
// ==========================================================================
// LA RUTA DEL RELÁMPAGO: sistema de niveles de Fixio
// Chispa -> Rayo -> Catatumbo (inspirado en el Relámpago del Catatumbo)
// A mayor nivel: menor comisión, mayor límite de wallet y mayor visibilidad.
// La wallet es la deuda de comisiones que el trabajador puede acumular antes
// de bloquearse (ya NO se bloquea por días, sino por monto acumulado).
// ==========================================================================
const NIVELES_TRABAJADOR = [
    {
        nivel: 1, nombre: 'Chispa', icono: 'sparkles', color: '#e0a935',
        comision: 10, limite_wallet: 15,
        // Requisitos para ascender al siguiente nivel:
        siguiente: { trabajos: 10, calificacion: 4.3, ganancias: 200 }
    },
    {
        nivel: 2, nombre: 'Rayo', icono: 'zap', color: '#6ba3f5',
        comision: 7, limite_wallet: 25,
        siguiente: { trabajos: 20, calificacion: 4.5, ganancias: 400 }
    },
    {
        nivel: 3, nombre: 'Catatumbo', icono: 'cloud-lightning', color: '#a78bfa',
        comision: 5, limite_wallet: 35,
        siguiente: null // nivel tope
    }
];

function configNivel(n) {
    return NIVELES_TRABAJADOR.find(x => x.nivel === n) || NIVELES_TRABAJADOR[0];
}

// El nivel se CALCULA siempre a partir de las estadísticas reales
// ("mantener" la calificación es literal: si baja del umbral, baja el nivel).
function calcularNivel(stats) {
    const n3 = NIVELES_TRABAJADOR[1].siguiente; // requisitos para llegar a 3
    const n2 = NIVELES_TRABAJADOR[0].siguiente; // requisitos para llegar a 2
    if (stats.trabajos >= n3.trabajos && stats.calificacion > n3.calificacion && stats.ganancias >= n3.ganancias) return 3;
    if (stats.trabajos >= n2.trabajos && stats.calificacion > n2.calificacion && stats.ganancias >= n2.ganancias) return 2;
    return 1;
}

async function obtenerEstadisticasTrabajador(trabajadorId) {
    const [[perfil]] = await pool.execute(
        'SELECT total_servicios, calificacion_promedio, nivel FROM perfiles_trabajador WHERE usuario_id = ?',
        [trabajadorId]
    );
    const [[gan]] = await pool.execute(
        `SELECT COALESCE(SUM(COALESCE(costo_final, costo_estimado)), 0) AS ganancias
         FROM solicitudes_servicio WHERE trabajador_id = ? AND estado = 'completado'`,
        [trabajadorId]
    );
    return {
        trabajos: Number(perfil?.total_servicios) || 0,
        calificacion: parseFloat(perfil?.calificacion_promedio) || 0,
        ganancias: parseFloat(gan.ganancias) || 0,
        nivel_guardado: Number(perfil?.nivel) || 1
    };
}

// Recalcula y guarda el nivel; si subió, se lo celebramos en vivo.
async function actualizarNivelTrabajador(trabajadorId) {
    try {
        const stats = await obtenerEstadisticasTrabajador(trabajadorId);
        const nuevo = calcularNivel(stats);
        if (nuevo !== stats.nivel_guardado) {
            await pool.execute('UPDATE perfiles_trabajador SET nivel = ? WHERE usuario_id = ?', [nuevo, trabajadorId]);
            if (nuevo > stats.nivel_guardado) {
                const cfg = configNivel(nuevo);
                emitirEvento(`user_${trabajadorId}`, 'nivel_subido', {
                    nivel: cfg.nivel, nombre: cfg.nombre, comision: cfg.comision, limite_wallet: cfg.limite_wallet
                });
            }
        }
        return nuevo;
...
// La wallet se bloquea al alcanzar el límite de deuda del nivel del trabajador
// para tomar nuevos trabajos hasta saldar TODA su deuda.
// Resumen de la WALLET del trabajador: deuda acumulada vs. límite de su nivel.
// Bloqueo cuando la deuda alcanza el límite (ya no por antigüedad).
async function obtenerDeudaTrabajador(trabajadorId) {
    ...
    const stats = await obtenerEstadisticasTrabajador(trabajadorId);
    const nivelActual = calcularNivel(stats);
    const cfg = configNivel(nivelActual);
    const cfgSiguiente = cfg.siguiente ? configNivel(nivelActual + 1) : null;

    const deuda_total = parseFloat(resumen.deuda_total) || 0;
    const margen = Math.max(0, cfg.limite_wallet - deuda_total);

    return {
        deuda_total,
        deuda_pendiente: parseFloat(resumen.deuda_pendiente) || 0,
        deuda_reportada: parseFloat(resumen.deuda_reportada) || 0,
        bloqueado: deuda_total >= cfg.limite_wallet,
        limite_wallet: cfg.limite_wallet,
        margen_disponible: Math.round(margen * 100) / 100,
        porcentaje: cfg.comision,
        // Nivel actual (Ruta del Relámpago)
        nivel: cfg.nivel,
        nivel_nombre: cfg.nombre,
        nivel_icono: cfg.icono,
        nivel_color: cfg.color,
        // Progreso hacia el siguiente nivel
        progreso: cfg.siguiente ? {
            siguiente_nombre: cfgSiguiente.nombre,
            trabajos: { actual: stats.trabajos, meta: cfg.siguiente.trabajos },
            calificacion: { actual: stats.calificacion, meta: cfg.siguiente.calificacion },
            ganancias: { actual: stats.ganancias, meta: cfg.siguiente.ganancias }
        } : null
    };
}
...
async function bloquearSiDeudaVencida(req, res, next) {
    try {
        const wallet = await obtenerDeudaTrabajador(req.user.id);
        if (wallet.bloqueado) {
            return res.status(403).json({
                message: `Wallet llena: acumulaste $${wallet.deuda_total.toFixed(2)} en comisiones y tu límite como nivel ${wallet.nivel_nombre} es $${wallet.limite_wallet}. Salda tu deuda en Perfil > Wallet Fixio para seguir tomando trabajos.`,
                bloqueado_por_deuda: true,
                deuda_total: wallet.deuda_total,
                limite_wallet: wallet.limite_wallet
            });
        }
        next();
    } catch (e) { next(); }
    ...
    try {
        const [rows] = await pool.execute(
            `SELECT u.id AS trabajador_id, u.nombre, u.apellido, u.email,
             ...
             LEFT JOIN perfiles_trabajador pt ON pt.usuario_id = u.id
             GROUP BY u.id, u.nombre, u.apellido, u.email, pt.nivel
             ORDER BY reportada DESC, pendiente DESC`
        );
        // Bloqueo por wallet: deuda >= límite del nivel de cada trabajador
        res.status(200).json(rows.map(r => {
            const cfg = configNivel(Number(r.nivel));
            const deuda = parseFloat(r.pendiente) + parseFloat(r.reportada);
            return {
                ...r,
                pendiente: parseFloat(r.pendiente), reportada: parseFloat(r.reportada),
                pagada: parseFloat(r.pagada),
                nivel_nombre: cfg.nombre, limite_wallet: cfg.limite_wallet,
                bloqueado: deuda >= cfg.limite_wallet
            };
        }));
    } catch (error) {
...
// Verificación de enlaces de la app Android (TWA / Play Store).
// El archivo assetlinks.json vincula tu dominio con la firma del APK.
app.get('/.well-known/assetlinks.json', (req, res) => {
    res.sendFile(path.join(__dirname, 'assetlinks.json'), (err) => {
        if (err) res.status(404).json({ message: 'assetlinks.json no configurado aún.' });
    });
});
...
// ==========================================================================
// ROBUSTEZ DE PRODUCCIÓN: errores globales y apagado elegante
// ==========================================================================
process.on('unhandledRejection', (razon) => {
    console.error('[ERROR NO MANEJADO - Promise]', razon);
});
process.on('uncaughtException', (err) => {
    console.error('[ERROR NO MANEJADO - Excepción]', err);
});

function apagarElegante(senal) {
    console.log(`\n[SYSTEM] ${senal} recibida: cerrando Fixio con elegancia...`);
    server.close(() => {
        Promise.resolve(pool && pool.end && pool.end()).finally(() => {
            console.log('[SYSTEM] Conexiones cerradas. Hasta pronto.');
            process.exit(0);
        });
    });
    setTimeout(() => process.exit(0), 5000); // salida forzada si algo se queda colgado
}
process.on('SIGINT', () => apagarElegante('SIGINT'));
process.on('SIGTERM', () => apagarElegante('SIGTERM'));
